...
//                self?.chaptersUploadHandle?()
    ...
    /// 当前所处章节数
    public var currentChapter = -999 {
        didSet {
            
//            guard chapterModels[currentChapter+1].path != nil else { return }
            
            // 解析下一章富文本数据
            let attriStr = self.attributedString(from: chapterModels[currentChapter+1])
            ...
            cutPage(with: attriStr!, config: SRReaderConfig.shared, completeHandler: { [weak self] (int, model, fin) in
                ...
                if fin {
                    self?.chapterModels[currentChapter+1].pageModels = pageModels.sorted { return $0.pageIndex < $1.pageIndex }
                }
            })
        }
    }
    ...
    ///                      [SRChapter]: 文章章节数组
    func parseTxtChapter(path: String, completeHandler: @escaping ([String], [SRReaderChapter]) -> Void) {
        ...
        DispatchQueue.global().async {
            ...
            if results.count == 0 { // 一个章节都没有
                ...
                for (index, result) in results.enumerated() {
                    ...
                    // 只加载第一章的具体数据
                    if index == 0 { self.currentChapter = -1 }
    ...
    /// - Returns: 章节富文本
    func attributedString(from chapter: SRReaderChapter) -> NSAttributedString? {
        ...
        let font = UIFont.init(name: SRReaderConfig.shared.fontName, size: SRReaderConfig.shared.fontSize)
    ...
    func cutPage(with attrString: NSAttributedString, config: SRReaderConfig, completeHandler: (_ pageCount: Int, _ model: SRReaderPage, _ isCompleted: Bool) -> Void) -> Void {
